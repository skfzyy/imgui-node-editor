...
//------------------------------------------------------------------------------
static void ImDrawListSplitter_Grow(ImDrawList* draw_list, ImDrawListSplitter* splitter,
                                    int channels_count)
{
...
static void ImDrawListSplitter_SwapChannels(ImDrawListSplitter* splitter, int left,
                                            int right)
{
...
//        }
//
//        for (auto vtx = vtxBuffer.Data + minIndex, vtxEnd = vtxBuffer.Data + maxIndex
//        + 1; vtx < vtxEnd; ++vtx)
//        {
...
// static void ImDrawList_ClampClipRects_Inner(ImVector<ImDrawCmd>& cmdBuffer, const
// ImVec4& clipRect, const ImVec2& offset)
//{
//    for (auto& cmd : cmdBuffer)
//    {
...
// static void ImDrawList_TranslateAndClampClipRects(ImDrawList* drawList, int begin,
// int end, const ImVec2& offset)
//{
//    int lastCurrentChannel = drawList->_ChannelsCurrent;
//    if (lastCurrentChannel != 0)
//        drawList->ChannelsSetCurrent(0);
...
/*
static void ImDrawList_PolyFillScanFlood(ImDrawList *draw, std::vector<ImVec2>* poly,
ImColor color, int gap = 1, float strokeWidth = 1.0f)
{
    ...
    while (y < max.y)
    {
...

        {
            ...
            for (size_t i = 0; i < polysize - 1; i++)
            {
                ...
                if ((pa.y > pb.y && y < pa.y && y > pb.y) || (pa.y < pb.y && y > pa.y &&
y < pb.y))
                {
                    ImVec2 intersect;
            ...
            sort(scanHits.begin(), scanHits.end(), [](ImVec2 const &a, ImVec2 const &b)
{ return a.x < b.x; });

            // generate the line segments.
            {
                int i = 0;
                int l = scanHits.size() - 1; // we need pairs of points, this prevents
segfault. for (i = 0; i < l; i += 2)
                {
...
static void ImDrawList_AddBezierWithArrows(ImDrawList* drawList,
                                           ...
                                           float thickness, float startArrowSize,
                                           float startArrowWidth, float endArrowSize,
                                           float endArrowWidth, bool fill, ImU32 color,
...
void ed::Node::Draw(ImDrawList* drawList, DrawFlags flags)
{
    if (flags == Detail::Object::None)
    {
        ...
        if (IsGroup(this))
        {
            drawList->AddRectFilled(m_GroupBounds.Min, m_GroupBounds.Max, m_GroupColor,
                                    m_GroupRounding);
...
ed::EditorContext::EditorContext(const ax::NodeEditor::Config* config)
    : m_IsFirstFrame(true), m_IsWindowActive(false), m_ShortcutsEnabled(true),
      m_Style(), m_Nodes(), m_Pins(), m_Links(), m_SelectionId(1),
      m_LastActiveLink(nullptr), m_Canvas(), m_IsCanvasVisible(false),
      m_NodeBuilder(this), m_HintBuilder(this), m_CurrentAction(nullptr),
      m_NavigateAction(this, m_Canvas), m_SizeAction(this), m_DragAction(this),
      m_SelectAction(this), m_ContextMenuAction(this), m_ShortcutAction(this),
      m_CreateItemAction(this),
      m_DeleteItemsAction(this), m_AnimationControllers{&m_FlowAnimationController},
      m_FlowAnimationController(this), m_DoubleClickedNode(0), m_DoubleClickedPin(0),
      m_DoubleClickedLink(0), m_BackgroundClicked(false),
      m_BackgroundDoubleClicked(false), m_IsInitialized(false), m_Settings(),
      m_Config(config), m_ExternalChannel(0)
{
}
...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    //    clipMin.x, clipMin.y, clipMax.x - clipMin.x, clipMax.y - clipMin.y, clipMax.x,
    //    clipMax.y);
...
void ed::EditorContext::End()
{
    ...
    // if (BackgroundDoubleClicked) LOG_TRACE(0, "DOUBLE CLICK BACKGROUND",
    // DoubleClickedLink);
    ...
    if (sortGroups ||
        ((m_Settings.m_DirtyReason & (SaveReasonFlags::Position |
                                      SaveReasonFlags::Size)) != SaveReasonFlags::None))
    {
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color,
                               float thickness)
{
    // auto& editorStyle = GetStyle();
...
void ed::EditorContext::UpdateAnimations()
{
    ...
    {
        const bool isLive = (std::find(m_LiveAnimations.begin(), m_LiveAnimations.end(),
...
void ed::EditorContext::ShowMetrics(const Control& control)
{
    ...
    auto liveNodeCount = (int)std::count_if(m_Nodes.begin(), m_Nodes.end(),
                                            [](Node* node) { return node->m_IsLive; });
    auto livePinCount = (int)std::count_if(m_Pins.begin(), m_Pins.end(),
                                           [](Pin* pin) { return pin->m_IsLive; });
    auto liveLinkCount = (int)std::count_if(m_Links.begin(), m_Links.end(),
...
ImVec2 ed::FlowAnimation::SamplePath(float distance)
{
    ...
    auto endPointIt =
        std::find_if(m_Path.begin(), m_Path.end(),
...
ed::NavigateAction::NavigateAction(EditorContext* editor, ImGuiEx::Canvas& canvas)
    : EditorAction(editor), m_IsActive(false), m_Zoom(1), m_Scroll(0, 0),
      m_ScrollStart(0, 0), m_ScrollDelta(0, 0), m_Canvas(canvas),
      m_WindowScreenPos(0, 0), m_WindowScreenSize(0, 0), m_Animation(editor, *this),
      m_Reason(NavigationReason::Unknown), m_LastSelectionId(0), m_LastObject(nullptr),
      m_MovingOverEdge(false), m_MoveOffset(0, 0)
{
}
...
bool ed::NavigateAction::HandleZoom(const Control& control)
{
    ...
    if (!io.MouseWheel ||
        (!allowOffscreen && !ImGui::IsWindowHovered())) // && !ImGui::IsAnyItemActive())
        return false;
...
void ed::NavigateAction::NavigateTo(const ImRect& bounds, bool zoomIn, float duration,
...
ed::SizeAction::SizeAction(EditorContext* editor)
    : EditorAction(editor), m_IsActive(false), m_Clean(false), m_SizedNode(nullptr),
      m_Pivot(NodeRegion::None), m_Cursor(ImGuiMouseCursor_Arrow)
{
}
...
void ed::SizeAction::ShowMetrics()
{
    ...
    if (m_SizedNode && m_IsActive)
    {
        ...
        ImGui::Text("    Start Bounds: { x=%g y=%g w=%g h=%g }", m_StartBounds.Min.x,
                    m_StartBounds.Min.y, m_StartBounds.GetWidth(),
...
bool ed::DragAction::Process(const Control& control)
{
    ...
    if (control.ActiveObject == m_DraggedObject)
    {
        ...
        // TODO: Move this experimental alignment to closes pivot out of internals to
        // node API
...
ed::ContextMenuAction::ContextMenuAction(EditorContext* editor)
    : EditorAction(editor), m_CandidateMenu(Menu::None), m_CurrentMenu(Menu::None),
      m_ContextId()
{
}
    ...
    if (candidateAction != None)
    {
        if (candidateAction != Paste && candidateAction != CreateNode)
        {
            ...
            const auto hasAnyLinks =
                std::any_of(m_Context.begin(), m_Context.end(),
...
ed::CreateItemAction::CreateItemAction(EditorContext* editor)
    : EditorAction(editor), m_InActive(false), m_NextStage(None), m_CurrentStage(None),
      m_ItemType(NoItem), m_UserAction(Unknown), m_LinkColor(IM_COL32_WHITE),
      m_LinkThickness(1.0f), m_LinkStart(nullptr), m_LinkEnd(nullptr),
...
void ed::NodeBuilder::Begin(NodeId nodeId)
{
    ...
    {
        ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(editorStyle.NodePadding.x,
